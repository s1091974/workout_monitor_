...
class PoseClassifier_crouch(
    ...
    companion object {
        private const val MODEL_FILENAME = "Crouch/pose_classifier.tflite"
        private const val LABELS_FILENAME = "Crouch/pose_labels.txt"
        private const val CPU_NUM_THREADS = 4

        fun create(context: Context): PoseClassifier_crouch {
            val options = Interpreter.Options().apply {
                setNumThreads(CPU_NUM_THREADS)
            }
            return PoseClassifier_crouch(
                Interpreter(
