...
class PoseClassifier_rightarm(
    ...
    companion object {
        private const val MODEL_FILENAME = "RightArm/pose_classifier.tflite"
        private const val LABELS_FILENAME = "RightArm/pose_labels.txt"
        private const val CPU_NUM_THREADS = 4

        fun create(context: Context): PoseClassifier_rightarm {
            val options = Interpreter.Options().apply {
                setNumThreads(CPU_NUM_THREADS)
            }
            return PoseClassifier_rightarm(
                Interpreter(
