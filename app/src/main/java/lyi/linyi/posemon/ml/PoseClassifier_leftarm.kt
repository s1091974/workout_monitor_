...
class PoseClassifier_leftarm(
    ...
    companion object {
        private const val MODEL_FILENAME = "LeftArm/pose_classifier.tflite"
        private const val LABELS_FILENAME = "LeftArm/pose_labels.txt"
        private const val CPU_NUM_THREADS = 4

        fun create(context: Context): PoseClassifier_leftarm {
            val options = Interpreter.Options().apply {
                setNumThreads(CPU_NUM_THREADS)
            }
            return PoseClassifier_leftarm(
                Interpreter(
