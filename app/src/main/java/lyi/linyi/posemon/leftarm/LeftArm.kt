...
import lyi.linyi.posemon.camera.CameraSource_left
...
import lyi.linyi.posemon.ml.PoseClassifier_leftarm
...
class LeftArm : AppCompatActivity() {
    companion object {
    ...
    private var cameraSource: CameraSource_left? = null
            ...
            if (cameraSource == null) {
                cameraSource =
                    CameraSource_left(
                        surfaceView,
                        selectedCamera,
                        object : CameraSource_left.CameraSourceListener {
...
//                                    }
                                ...
                                if (timeCounter >= 0.5) {
                                    ...
                                    if (left_round ==2) {
                                        ...
                                        finishPlayer.start()
                                        //資料庫回傳直放這
                                        val leftRoundRecord = LeftRoundRecord(leftRound = left_round)
                                        val database = RoundDatabase.getDatabase(applicationContext)
                                        database.leftRoundRecordDao().insert(leftRoundRecord)
    ...
    private fun isPoseClassifier() {
        cameraSource?.setClassifier(if (isClassifyPose) PoseClassifier_leftarm.create(this) else null)
    }
