...
import lyi.linyi.posemon.camera.CameraSource_crouch
...
import lyi.linyi.posemon.ml.PoseClassifier_crouch
...
class Crouch : AppCompatActivity() {
    companion object {
    ...
    private var cameraSource: CameraSource_crouch? = null
            ...
            if (cameraSource == null) {
                cameraSource =
                    CameraSource_crouch(
                        surfaceView,
                        selectedCamera,
                        object : CameraSource_crouch.CameraSourceListener {
...
//                                    }
                                ...
                                if (timeCounter >= 5) {
                                    ...
                                    if (crouch_round == 2) {
                                        ...
                                        finishPlayer.start()
                                        //資料庫回傳直放這
                                        val crouchRoundRecord = CrouchRoundRecord(crouchRound = crouch_round)
                                        val database = RoundDatabase.getDatabase(applicationContext)
                                        database.crouchRoundRecordDao().insert(crouchRoundRecord)
    ...
    private fun isPoseClassifier() {
        cameraSource?.setClassifier(if (isClassifyPose) PoseClassifier_crouch.create(this) else null)
    }
