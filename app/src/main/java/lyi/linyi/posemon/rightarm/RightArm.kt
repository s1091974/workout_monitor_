...
package lyi.linyi.posemon.rightarm
...
import lyi.linyi.posemon.camera.CameraSource_right
...
import lyi.linyi.posemon.ml.PoseClassifier_rightarm
import android.widget.Button
//import lyi.linyi.posemon.history.RightRecord.RightArmViewModel
...
class RightArm : AppCompatActivity() {
    companion object {
    ...
    private var cameraSource: CameraSource_right? = null
...
//        val rightArmViewModel = ViewModelProvider(this).get(RightArmViewModel::class.java)
//        rightArmViewModel.getRightArmData().observe(this) { round_right ->
//
//
//        }
        /** 程序运行时保持屏幕常亮 */
            ...
            if (cameraSource == null) {
                cameraSource =
                    CameraSource_right(
                        surfaceView,
                        selectedCamera,
                        object : CameraSource_right.CameraSourceListener {
...
//                                    }
                                ...
                                if (timeCounter >= 5) {
                                    ...
                                    if (round_right == 4) {
                                        ...
                                        //資料庫回傳直放這
                                        val roundRecord = RoundRecord(rightRound = round_right)
                                        val database = RoundDatabase.getDatabase(applicationContext)
                                        database.roundRecordDao().insert(roundRecord)
                                        //播放完成音檔
                                        finishPlayer.start()
                                        cameraSource?.close()
                                        cameraSource = null
                                        val message: String? = "恭喜!"
...
//        val rightHandRound = round_right

    }

    private fun isPoseClassifier() {
        cameraSource?.setClassifier(if (isClassifyPose) PoseClassifier_rightarm.create(this) else null)
    }
